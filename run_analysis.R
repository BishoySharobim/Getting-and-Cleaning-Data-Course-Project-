...
#Set working directory and reading data in.
    setwd("C:/Users/User/Google Drive/Career/Data Analysis/R/Coursera/3) Getting and cleaning data/Assignment/UCI HAR Dataset")

    train <- read.table("./train/X_train.txt")
    trainlbls <- read.table("./train/y_train.txt")
    test <- read.table("./test/X_test.txt")
    testlbls <- read.table("./test/y_test.txt")
    features <- read.table("features (U).txt")
